...
{
	if ( context->narrative.mode.one || context->narrative.mode.script )
	{
		*next_state = "";
	}
	else
...
#define RETURN ( context->narrative.mode.one ? out : base )
	...
	in_( base ) bgn_
		...
		in_( "!" ) bgn_
			...
			in_( "!." ) bgn_
				...
				in_( "!. expression" )
					if ( context->narrative.mode.one ) bgn_

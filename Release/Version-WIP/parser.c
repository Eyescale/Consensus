...
EXPR_BGN:CND_endif
	...
	in_( "expr" ) bgn_
		ons( " \t" )	do_( same )
		on_( '\n' ) if ( is_f(VECTOR|SET|CARRY) ) { // allow \nl inside <> {} and carry()
				do_( "_\\n" ) }
		...
		on_( ',' ) if ( !is_f(INFORMED) || is_f(TERNARY) )
				...
				do_( "{_," )	f_clr( NEGATED|FILTERED ) } // do not clear FIRST|INFORMED
			else if ( *type&DO ) {
				do_( "_," )	f_clr( FIRST|INFORMED|NEGATED|FILTERED ) }
				...
				if ( is_f(TERNARY) ) {
					if ( is_f(PRIMED) ) {
						do_( same )	s_take
								...
								f_pop( stack, 0 ) // never carry MARKED
								f_set( INFORMED ) } }
				else if ( is_f(INFORMED) ) {
						do_( same )	s_take
								f_pop( stack, is_f(LEVEL) ?
									MARKED|INFORMED : INFORMED ) } }
		...
		on_( '}' ) if ( *type&(DO|ON_X) && are_f(INFORMED|SET) && !is_f(LEVEL|SUB_EXPR) ) {
				do_( same )	s_take
						...
						f_tag( stack )
						f_set( INFORMED ) }
		on_( '|' ) if ( is_f(INFORMED) && is_f(LEVEL|SET) && *type&DO && f_actionable(stack) ) {
				do_( "|" )	s_take
						f_tag( stack )
						f_clr( INFORMED ) }
		on_( '\'' ) if ( !is_f(INFORMED) ) {
		...
		in_other bgn_
			on_( '\n' ) bgn_
				in_( "expr" )	errnum = ( is_f( MARKED ) ?
							(*type&ON && opt(RELEASED)) ? ErrMarkOn :
...
char *
bm_load( int event, BMParseMode mode, BMParseData *data, BMParseCB cb )
/*
...
*/
{
	...
	BMParseBegin( "bm_load", state, event, line, column )
	//----------------------------------------------------------------------
	// bm_load:	Expression states
	//----------------------------------------------------------------------
	in_( "base" ) bgn_
		on_( EOF )	do_( "" )
		ons( " \t\n" )	do_( same )
...
bm_parse_report( BMParseData *data, BMParseMode mode, int l, int c )
{
	...
	char *src = (mode==BM_STORY) ? "bm_parse" :
		    (mode==BM_INPUT) ? "bm_input" : "bm_load";

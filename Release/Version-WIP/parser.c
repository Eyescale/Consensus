...
EXPR_BGN:CND_endif
	...
	in_( "expr" ) bgn_
		...
		on_( ',' ) if ( !is_f(INFORMED) || is_f(TERNARY) )
				...
				do_( "," )	f_clr( NEGATED|FILTERED ) } // do not clear FIRST
			else if ( is_f(ASSIGN) ) {
				if ( !is_f(FILTERED) ) {
					do_( same )	s_take
...
A:CND_else_( B )
		...
		on_( '<' ) if ( is_f(SET|LEVEL|SUB_EXPR|EENOV|VECTOR) )
				...
				if ( is_f(ELLIPSIS) && !is_f(INFORMED|NEGATED|FILTERED) ) {
					do_( same )	s_take
							f_push( stack )
							f_reset( FIRST|VECTOR, 0 ) }
				else if ( is_f(ASSIGN) && !is_f(INFORMED|NEGATED) ) {
					do_( ":<" )	s_take }
				else if ( is_f(INFORMED) && !is_f(ASSIGN|ELLIPSIS) ) {
					do_( "_<" )	s_take } }
			else if ( *type&OUTPUT && !is_f(INFORMED|NEGATED|FILTERED) ) {
				do_( same )	s_take
						f_push( stack )
						f_reset( FIRST|VECTOR, 0 ) }
...
B:CND_endif
		...
		on_( '{' ) if ( is_f(INFORMED|SUB_EXPR|NEGATED|FILTERED|VECTOR|EENOV|ELLIPSIS) )
				; // err
			else if ( *type&DO ? !is_f(ASSIGN) : ( *type&ON_X && !is_f(LEVEL|SET) )) {
				do_( same )	s_take
						f_push( stack )
						f_reset( FIRST|SET, 0 ) }
	...
	in_( "," ) bgn_ // Assumption: INFORMED is set
		ons( " \t" )	do_( same )
...
CND_ifn( mode==BM_STORY, C )
	...
	in_( ":<" ) bgn_ // Assumption: ASSIGN is set
		ons( " \t" )	do_( same )
		on_( '\n' ) if ( !is_f(FILTERED) ) {
				do_( "expr" )	REENTER

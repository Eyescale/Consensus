...
static void 	r_register( CNString **, int );
static Rule *	r_set( CNString **, Scheme * );
static void	s_register( CNString **, int, int * );
static void 	s_finish( CNString **, Rule *, int *, listItem ** );
static void	t_register( CNString **, int );
...
readSchemeFrame( ReadSchemeData *data, int event )
{
	...
	in_( "rule" ) bgn_
		...
		on_other	do_( same )	r_register( string, event );
		end
		in_( "rule_" ) bgn_
			on_space	do_( same )
			on_( ':' )	do_( "schema" )	rule = r_set( string, scheme );
	...
	in_( "schema" ) bgn_
		...
		on_( '\n' )	do_( ": \\n" )
		on_( '\\' )	do_( ": \\" )
		on_other	do_( ": _" )	s_register( string, event, &position );
		end
		in_( ": space" ) bgn_
			on_space	do_( same )
			on_( '%' )	do_( ": space%" )
			...
			on_( '\\' )	do_( ": \\" )	rx_position = 0;
							if ( isRuleBase( rule ) || ( string[ SCHEMA ]->data ))
								s_register( string, ' ', &position );
			on_other	do_( ": _" )	rx_position = 0;
							if ( isRuleBase( rule ) || ( string[ SCHEMA ]->data ))
								s_register( string, ' ', &position );
							s_register( string, event, &position );
			end
		in_( ": space%" ) bgn_
		...
		in_( ": \\" ) bgn_
			on_( '\n' )	do_( "schema" )
			on_other	do_( "schema" )	s_register( string, '\\', &position );
							s_register( string, event, &position );
			end
		in_( ": _" ) bgn_
			on_separator	do_( "schema" )	REENTER
			on_other	do_( same )	s_register( string, event, &position );
			end
		in_( ": %" ) bgn_
			on_( '{' )	do_( "tokens" )
			on_( '-' )	do_( ": %-" )
			on_( '/' )	do_( ": regex" )	s_register( string, '%', &position );
								rx_position = position;
								s_register( string, '/', &position );
			on_( '\n' )	do_( ": \\n" )		errnum = WarnReadSchemeExtra;
			on_space	do_( ": % " )		s_register( string, '%', &position );
								s_register( string, ' ', &position );
			on_separator	do_( "schema" )		s_register( string, '%', &position );
								s_register( string, event, &position );
			on_other	do_( ": %_" )		s_register( string, '%', &position );
								s_register( string, event, &position );
			end
			in_( ": % " ) bgn_
				on_space	do_( same )
		...
		in_( ": regex" ) bgn_
			on_( '\n' )	do_( "error" )	REENTER
							rx_position = 0;
			on_( '\\' )	do_( ": %/\\" )
			on_( '/' )	do_( ": %//" )	s_register( string, event, &position );
			on_other	do_( same )	s_register( string, event, &position );
			end
			in_( ": %/\\" ) bgn_
				on_( '\n' )	do_( ": %/\\\\n" )
				on_other	do_( ": regex" )	s_register( string, '\\', &position );
									s_register( string, event, &position );
				end
				in_( ": %/\\\\n" ) bgn_
					on_space	do_( same )
	...
	in_( "tokens" ) bgn_
		...
		on_other	do_( same )		t_register( string, event );
		end
		in_( "token " ) bgn_
			on_space	do_( same )
		...
		in_( "token\\" ) bgn_
			on_( '\n' )	do_( "tokens" )
			on_other	do_( "tokens" )	t_register( string, '\\' );
							t_register( string, event );
			end
		in_( "%{_}" ) bgn_
			on_space	do_( ": space" )
			on_( '%' )	do_( "%{_}%" )
...
static void
r_register( CNString **s, int event )
{
...
static void
s_register( CNString **s, int event, int *position )
{
...
static void
t_register( CNString **s, int event )
{
